...
class PopularityDataBias(DataBiasStrategy):
    def run(self, strategy_context: DataBiasStrategyContext) -> Graph:
        ...
        number_of_ratings_per_movie: pandas.Series = movies.value_counts(sort=False)
        number_of_ratings_per_rating: pandas.Series = number_of_ratings_per_movie.value_counts(sort=False).sort_index()
